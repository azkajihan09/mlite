...
class Admin extends AdminModule
{
  ...
  public function getRiwayat($no_rawat, $status = null)
  {
    ...
    $kd_dokter_bpjs = null;
    if($status == 'fktp') {
      $mapping_dokter = $this->db('maping_dokter_pcare')
        ->where('kd_dokter', $this->core->getRegPeriksaInfo('kd_dokter',revertNoRawat($no_rawat)))
        ->oneArray();
      if (is_array($mapping_dokter) && isset($mapping_dokter['kd_dokter_pcare'])) {
        $kd_dokter_bpjs = $mapping_dokter['kd_dokter_pcare'];
      }
    } else {
      $mapping_dokter = $this->db('maping_dokter_dpjpvclaim')
        ->where('kd_dokter', $this->core->getRegPeriksaInfo('kd_dokter',revertNoRawat($no_rawat)))
        ->oneArray();
      if (is_array($mapping_dokter) && isset($mapping_dokter['kd_dokter_bpjs'])) {
        $kd_dokter_bpjs = $mapping_dokter['kd_dokter_bpjs'];
      }
    }
    ...
    $data = [
      ...
      'kodedokter' => intval($kd_dokter_bpjs ?: 0)
    ];
    ...
    $riwayat_url = '';
    $message = 'Gagal memuat riwayat iCare';

    if (is_array($json) && isset($json['metaData']['code'])) {
      if ((string)$json['metaData']['code'] === '200') {
        $stringDecrypt = stringDecrypt($key, isset($json['response']) ? $json['response'] : '');
        if (!empty($stringDecrypt)) {
          $decompress = LZString::decompressFromEncodedURIComponent($stringDecrypt);
          $riwayat = json_decode($decompress, true);
          if (is_array($riwayat) && isset($riwayat['url'])) {
            $riwayat_url = (string)$riwayat['url'];
          }
        }
        $message = '200';
      } else {
        $message = isset($json['metaData']['message']) ? (string)$json['metaData']['message'] : 'Gagal memuat riwayat iCare';
      }
    }

    echo $this->draw('riwayat.html', [
      'url' => htmlspecialchars($riwayat_url, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'),
